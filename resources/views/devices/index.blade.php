...
@section('content')
    <div class="row">
        <div class="col-lg-12">
            <x-card>
                <x-slot name="header">
                    <button onclick="addForm(`{{ route('devices.store') }}`)" class="btn btn-sm btn-primary"><i
...
@push('scripts')
    <script>
        ...
        // tambah data
        function addForm(url, title = 'Form Tambah Data Alat') {
            $(modal).modal('show');
        ...
        // edit data
        function editForm(url, title = 'Form Edit Data Alat') {
            $.get(url)
                .done(response => {
                    $(modal).modal('show');
                    $(`${modal} .modal-title`).text(title);
